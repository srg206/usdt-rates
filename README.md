...
**Сейчас:** исходящие запросы к бирже ограничены семафором (`GRINEX_MAX_CONCURRENT`); при сбоях срабатывает circuit breaker. Каждый `GetRates` идёт **on-demand** в API и пишет снимок в БД — максимально свежий курс в обмен на каждый вызов к Grinex.

**Что можно улучшить**

- **Документация лимитов** — зафиксировать в runbook/деплое целевые concurrency/RPS и согласование с реальными лимитами Grinex; проверить, что семафор не создаёт недопустимых очередей для вашего SLA.
- **Стратегия свежести** — при необходимости реже бить в API: фоновый polling + отдача последнего снимка из `GetRates` (явный TTL, описание в README для потребителей). Для «курс строго на момент запроса» оставить on-demand + семафор + breaker.
- **Масштабирование** — рост числа подов с учётом лимитов по IP/ключу; при необходимости **pod anti-affinity**, чтобы разнести реплики по нодам. Обход лимитов через эмуляцию «разных клиентов» (разные заголовки и т.п.) — риск по ToS; предпочтительнее официальные лимиты, ключи, **WebSocket/streaming**, если есть.
- **Наблюдаемость** — метрики и алерты на **429**, учёт **Retry-After**, доля ошибок и latency к бирже.
- **Ретраи** — экспоненциальный backoff и jitter, ограничение числа попыток.
- **Дедупликация** — single-flight или короткий кэш для одинаковых параллельных запросов.
- **Глобальная квота** — при многих репликах один локальный семафор может быть недостаточен; тогда централизованный лимит (сервис, sidecar, Redis и т.д.).
